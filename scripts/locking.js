...
async function main() {
    ...
    // await hre.ethers.provider.send('evm_setNextBlockTimestamp', [1645634753 + 5 + 2 * 24 * 3600]); //rewards started + 1 day
    ...
    var receipt = await re.wait();
    console.log(receipt.blockNumber);
    await hre.ethers.provider.send('evm_mine');
    re = await ve.create_lock(ve_underlying_amount, lockDuration * 2);
    receipt = await re.wait();
    console.log(receipt.blockNumber);
    // console.log(ve.balanceOfN);
    await hre.ethers.provider.send('evm_mine');
    await hre.ethers.provider.send('evm_mine');

    console.log(await ve.totalSupplyAt(receipt.blockNumber + 1));
    // console.log(receipt);
    await hre.ethers.provider.send('evm_setNextBlockTimestamp', [rewards_starts_at + 5 + 2 * 24 * 3600]); //rewards started + 1 day
    ...
    await hre.ethers.provider.send('evm_mine');
    await hre.ethers.provider.send('evm_mine');
    await hre.ethers.provider.send('evm_setNextBlockTimestamp', [rewards_starts_at + 500 + 2 * 24 * 3600]); //rewards started + 1 day
    await hre.ethers.provider.send('evm_mine');
    await hre.ethers.provider.send('evm_mine');

    console.log(await ve.totalSupplyAtT(rewards_starts_at + 5400000 + 2 * 24 * 3600));
    // console.log(await ve.totalSupplyAt(receipt.blockNumber + 1));
}
